...
struct Obstacle {
    ...
    Obstacle(QVector2D start, QVector2D end, qreal thickness, int id){
        // Obstacle object cosntructor
        ...
        qDebug() << "Obstacle" << id << "line:" << this->line ;
        ...
        qDebug() << "Obstacle" << id << "normal:" << this->normal;
        ...
        qDebug() << "Obstacle" << id << "unitary:" << this->unitary;
        qDebug("Setting Obstacle graphics line...");
...
TransmitterTP4 TX(9.4,7); // initialized global TX
ReceiverTP4 RX(7, 7.5); // initialized global RX
...
void addReflectionComponents(QGraphicsScene* scene, const QVector2D& RX, const QVector2D& TX, const QVector2D& r_image, const QVector2D& r_image_image) {
    ...
    double sin_theta_t = sin_theta_i / sqrt(wall->properties.relative_permittivity);
    ...
    qDebug() << "s" << s;
    complex<double> Gamma_perpendicular = (wall->properties.Z_m * cos_theta_i - Z_0 * cos_theta_t) / (wall->properties.Z_m * cos_theta_i + Z_0 * cos_theta_t);
    ...
    complex<double> reflection_term = exp(-2.0 * wall->properties.gamma_m * s) * exp(j * 2.0 * beta_0 * s * sin_theta_t * sin_theta_i);
    ...
    double sin_theta_t_2 = sin_theta_i_2 / sqrt(wall_2->properties.relative_permittivity);
    double cos_theta_t_2 = sqrt(1 - pow(sin_theta_t_2, 2));
    double s_2 = wall_2->thickness / cos_theta_t_2;
    complex<double> Gamma_perpendicular_2 = (wall_2->properties.Z_m * cos_theta_i_2 - Z_0 * cos_theta_t_2) / (wall_2->properties.Z_m * cos_theta_i_2 + Z_0 * cos_theta_t_2);
    // TODO : valeur un petit peu différente de la valeur attendue, pourquoi ?
    complex<double> reflection_term_2 = exp(-2.0 * wall_2->properties.gamma_m * s_2) * exp(j * 2.0 * beta_0 * s_2 * sin_theta_t_2 * sin_theta_i_2);
...
complex<qreal> computeReflectionCoeff(qreal _cos_theta_i, qreal _sin_theta_i, qreal _cos_theta_t, qreal _sin_theta_t, Obstacle* wall)
{
    ...
    qDebug() << "s=" << s;
    qDebug() << "gamma_m=" << wall->properties.gamma_m.real() << "+j" << wall->properties.gamma_m.imag();
    complex<qreal> Gamma_perpendicular = computePerpendicularGamma(_cos_theta_i, _cos_theta_t, wall);
    complex<qreal> first_exp = exp(-2.0 * wall->properties.gamma_m * s);
...
bool checkRaySegmentIntersectsWall(const Obstacle* wall, RaySegmentTP4* ray_segment, QPointF* intersection_point=nullptr) {
...
qreal makeTransmission(RaySegmentTP4* ray_segment, Obstacle* wall) {
...
void checkTransmissions(RayTP4* _ray, QList<Obstacle*> _reflection_walls) {
...
void addReflection(RayTP4* _ray, const QVector2D& _p1, const QVector2D& _p2, Obstacle* wall){
...
void computeReflections(const QVector2D& _RX, const QVector2D& _TX)
{
    ...
    // 1st reflection :
    //for (Obstacle* wall: wall_list) { // could use this instead
    for (int i=0; i<wall_list.length(); i++) {
        Obstacle* wall = wall_list[i];
